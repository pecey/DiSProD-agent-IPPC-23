...
def compute_avg_action_time(queue, env, cfg, cfg_env, g_obs_keys, ga_keys, mode="complete", depth=20):
    fn_start = time.time()
    cfg["depth"] = depth
    ...
    for step in range(depth):
        ...
        print(f"[Time: {finish-start}] [H-{mode}-{depth}] Action generated {action}")
        times.append(finish-start)
    del agent
    fn_end = time.time()
    print(f"[Time: {fn_end-fn_start}] [H-{mode}-{depth}] Completed heuristic scan {action}")
    queue.put(np.mean(times), (mode, depth))