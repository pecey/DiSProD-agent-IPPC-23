...
env_name="Reservoir continuous"
jids=()

echo "Evaluating ${env_name}"
SLURM_LOG_PATH=/N/u/palchatt/BigRed3/IPC23/slurm_logs/${env_name}/${identifier}
mkdir -p ${SLURM_LOG_PATH}

# for mode in "${modes[@]}"; do
for inst in "${insts[@]}"; do
    slurm_log_file=${inst}
    jid=$(sbatch --export=ALL,base_run_name=${identifier},env=${env_name},inst=${inst} --parsable -J e-${env_name}-${inst} -o ${SLURM_LOG_PATH}/${slurm_log_file}.out  -e ${SLURM_LOG_PATH}/${slurm_log_file}.err /N/u/palchatt/BigRed3/IPC23/evaluate/eval.sh)
