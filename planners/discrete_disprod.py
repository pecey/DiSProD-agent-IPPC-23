...
class DiscreteDisprod(Disprod):
    ...
    def diag_of_hessian(self, s, a, wrt):
        if self.nn_model:
            stacked_hessian = self.hessian(self.dynamics_fn, wrt)(s, a)
        else:
    ...
    def initialize_conformant_actions(self):
        ...
        extra_restarts = self.n_res - (n_batches * self.nA)
    ...
    def choose_action(self, obs):
        ...
        initial_state = jnp.tile(state, (self.n_res, 1)).astype('float32')
    ...
    def next_state_for_nn(self, operands):
        ...
        return self.dynamics_fn(state, final_actions, model_params)

    def next_state_for_exact_fn(self, operands):
        state, actions, _ = operands
        return self.dynamics_fn(state, actions, self.env)
